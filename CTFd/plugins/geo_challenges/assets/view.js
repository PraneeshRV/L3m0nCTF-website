...
CTFd.plugin.run((_CTFd) => {
  ...
  function updateSubmissionInput() {
    const lat = $("#submit-latitude").val();
    const lon = $("#submit-longitude").val();
    
    if (lat && lon) {
      const submissionValue = `lat:${lat},lon:${lon}`;
      $("#challenge-input").val(submissionValue);
    }
  }
  ...
  function loadSolves() {
    ...
    _CTFd.fetch(`/api/v1/challenges/${challengeId}/solves`, {
      method: "GET",
      credentials: "same-origin",
      headers: {
        Accept: "application/json",
    ...
    .then((response) => response.json())
    .then((data) => {
      const solvesContainer = $("#challenge-solves-names");
      solvesContainer.empty();
      ...
      if (data.success && data.data && data.data.length > 0) {
        data.data.forEach((solve) => {
          ...
          `);
          solvesContainer.append(row);
        });
      } else {
        solvesContainer.append(`
          <tr>
